...
# Source ~/.env and export all variables for child processes (MCP servers, etc.)
if [ -f "$HOME/.env" ]; then
  set -a
  source "$HOME/.env"
  set +a
...
# If this shell is NOT a login shell, import ~/.zprofile once.
if [[ ! -o login && -r ~/.zprofile ]]; then
  # Optional flag so ~/.zprofile can tell it's being sourced from .zshrc
  export ZPROFILE_SOURCED_FROM_ZSHRC=1
  source ~/.zprofile
fi
...
alias tree="eza --tree"
alias lla="eza -la"
mdd() { [[ ! -f "$1" ]] && touch "$1"; typora "$@"; }
alias meeting-prep='python3 ~/.claude/skills/meeting-prep/scripts/meeting_prep.py'
...
# export ANTHROPIC_CUSTOM_HEADERS='Send-Raw-Response-Header: true'
...
if [ -f '/Users/muqi.li/Downloads/google-cloud-sdk/path.zsh.inc' ]; then . '/Users/muqi.li/Downloads/google-cloud-sdk/path.zsh.inc'; fi
